/**
 * WAVES SPHERE - Complete Bundle
 * A self-contained animated 3D sphere with wave effects
 * 
 * Dependencies: Three.js (must be loaded separately)
 * 
 * Usage:
...
(function(global) {
    ...
    if (typeof THREE === 'undefined') {
        throw new Error('WavesSphere requires Three.js to be loaded first. Please include Three.js before this script.');
    }
    ...
    class WavesSphere {
        ...
        createBackgroundSphere() {
            const geometry = new THREE.SphereGeometry(this.config.radius * 0.95, 64, 32);
        ...
        zoomTo(distance, smooth = false) {
            const targetDistance = Math.max(this.config.minZoom, Math.min(this.config.maxZoom, distance));
...
void main() {
    ...
    if (typeof module !== 'undefined' && module.exports) {
        // Node.js
        module.exports = WavesSphere;
    } else if (typeof define === 'function' && define.amd) {
        // AMD
        define([], function() { return WavesSphere; });
    } else {
        // Browser global
        global.WavesSphere = WavesSphere;
    }
