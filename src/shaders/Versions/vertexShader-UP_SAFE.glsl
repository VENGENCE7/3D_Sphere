...
void main() {
    ...
    vec3 brightRed = vec3(1.0, 0.25, 0.2);         // #FF4033 - Bright red
    ...
    // === LOWER HEMISPHERE GRADIENT - Clean modular structure ===
    // Color sequence: Black → Deep Purple → Red → Deep Purple → Wine Purple → Red → Yellow
    
    // Calculate lower latitude factor (1.0 at south pole, 0.0 at equator)
    float lowerLatitudeFactor = clamp(-verticalPosition, 0.0, 1.0);
    
    // Dynamic band positions based on latitude
    // Yellow rim: 3% at south pole, reduces toward equator
    float lowerGoldenStart = mix(0.99, 0.97, lowerLatitudeFactor); // 1% at equator, 3% at south pole
    float lowerYellowStart = mix(0.95, 0.85, lowerLatitudeFactor); // Yellow zone
    float lowerRedStart2 = mix(0.90, 0.70, lowerLatitudeFactor);   // Second red
    float lowerWineStart = mix(0.85, 0.55, lowerLatitudeFactor);   // Wine purple
    float lowerDeepStart2 = mix(0.75, 0.45, lowerLatitudeFactor);  // Second deep purple
    float lowerRedStart1 = mix(0.65, 0.35, lowerLatitudeFactor);   // First red
    float lowerDeepEnd1 = mix(0.50, 0.20, lowerLatitudeFactor);    // First deep purple end
    
    // Clean band transitions for lower hemisphere
    if (eclipseFactor <= 0.08) {
        // Band 0: Black core (0.00-0.08)
        lowerColor = getColorInBand(eclipseFactor, 0.0, 0.08, centerBlack, centerBlack * 1.2);
    }
    else if (eclipseFactor <= lowerDeepEnd1) {
        // Band 1: Black to Deep Purple (0.08-0.20/0.50)
        lowerColor = getColorInBand(eclipseFactor, 0.08, lowerDeepEnd1, centerBlack, deepPurple);
    }
    else if (eclipseFactor <= lowerRedStart1) {
        // Band 2: Deep Purple to Red (fades out near equator)
        if (lowerLatitudeFactor > 0.3) {
            // Red visible at poles
            vec3 targetRed = mix(deepPurple, brightRed, lowerLatitudeFactor);
            lowerColor = getColorInBand(eclipseFactor, lowerDeepEnd1, lowerRedStart1, deepPurple, targetRed);
        } else {
            // Near equator: stay in deep purple
            lowerColor = deepPurple * 1.1;
        }
    }
    else if (eclipseFactor <= lowerDeepStart2) {
        // Band 3: Red to Deep Purple (only at poles)
        if (lowerLatitudeFactor > 0.4) {
            vec3 sourceRed = mix(deepPurple, brightRed, lowerLatitudeFactor);
            lowerColor = getColorInBand(eclipseFactor, lowerRedStart1, lowerDeepStart2, sourceRed, deepPurple * 1.2);
        } else {
            // Near equator: continue deep purple
            lowerColor = deepPurple * 1.2;
        }
    }
    else if (eclipseFactor <= lowerWineStart) {
        // Band 4: Deep Purple to Wine Purple (0.45/0.75-0.55/0.85)
        lowerColor = getColorInBand(eclipseFactor, lowerDeepStart2, lowerWineStart, deepPurple * 1.2, winePurple);
    }
    else if (eclipseFactor <= lowerRedStart2) {
        // Band 5: Wine Purple to Red (fades near equator)
        if (lowerLatitudeFactor > 0.2) {
            // Red intensity decreases toward equator
            vec3 targetRed2 = mix(winePurple, brightRed * 1.1, lowerLatitudeFactor * 1.5);
            lowerColor = getColorInBand(eclipseFactor, lowerWineStart, lowerRedStart2, winePurple, targetRed2);
        } else {
            // Near equator: stay in wine purple
            lowerColor = winePurple * 1.1;
        }
    }
    else if (eclipseFactor <= lowerYellowStart) {
        // Band 6: Red/Purple to Yellow transition
        if (lowerLatitudeFactor > 0.3) {
            // At poles: red to yellow
            vec3 sourceColor = mix(winePurple, brightRed * 1.1, lowerLatitudeFactor);
            lowerColor = getColorInBand(eclipseFactor, lowerRedStart2, lowerYellowStart, sourceColor, goldenYellow);
        } else {
            // Near equator: wine purple to yellow
            lowerColor = getColorInBand(eclipseFactor, lowerRedStart2, lowerYellowStart, winePurple * 1.1, goldenYellow * 0.8);
        }
    }
    else if (eclipseFactor <= lowerGoldenStart) {
        // Band 7: Yellow zone (0.85/0.95-0.97/0.99)
        lowerColor = getColorInBand(eclipseFactor, lowerYellowStart, lowerGoldenStart, goldenYellow, goldenYellow * 1.1);
    }
    else {
        // Band 8: Golden corona (0.97/0.99-1.00)
        float t = smootherstep(lowerGoldenStart, 1.0, eclipseFactor);
        lowerColor = mix(goldenYellow * 1.1, brightGold, t);
    }
